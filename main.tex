...
Training diagnostics were monitored to avoid unstable policies. Pilot runs revealed over-waiting near end of life; the final configuration resolved this through stronger crash penalties and end-of-life start biasing.
...
Behavioural checks complemented functional tests. Reward-outcome mapping and uncertainty monotonicity were validated.
...
Under the full three-layer system, UA produced lower average cost per decision than blind at all eight noise levels: \pounds36,863 versus \pounds39,648 (\pounds2,785, or 7.0\%, saved per decision). At fleet level, this corresponds to \pounds556,950 annual savings and \pounds2,784,750 over five years, with positive savings retained even at heavy noise.
...
\begin{enumerate}
  ...
  \item \textbf{The DQN outperformed fixed-threshold baselines.} The learned policy adapted its behaviour to the current noise regime, a capability that no static threshold could replicate, confirming that reinforcement learning added genuine value to the maintenance scheduling task.
    ...
    \item \textbf{Uncertainty awareness improved fleet-level cost-effectiveness.} Under the calibrated full-system cost model, UA achieved lower annual fleet cost than blind at all eight noise levels, yielding \pounds556,950 annual savings (\pounds2.78M over five years) for the representative 50-engine fleet.
\end{enumerate}
...
Appendix~\ref{app:visualisations} presents supplementary figures that provide additional perspectives on the experimental results and ensure that all generated visual outputs used for evaluation are archived in the dissertation. Figure~\ref{fig:app_exp1_alt} shows the Layer~2 ablation from an alternative angle, Figure~\ref{fig:app_exp2_alt} provides an alternative view of full-system performance, and Figures~\ref{fig:app_cost_full} and \ref{fig:app_cost_exec} provide the full financial analysis visualisations.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{report_cost_analysis.png}
    \caption{Detailed cost-effectiveness figure generated by the financial evaluation pipeline, including per-decision costs, annual savings, and outcome composition across noise levels.}
    \label{fig:app_cost_full}
\end{figure}
