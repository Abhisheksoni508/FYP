...
\begin{titlepage}
    ...
    {\LARGE \textbf{A Robust Predictive Maintenance Framework for IoT Systems via Uncertainty-Aware Reinforcement Learning}} \\[0.5cm]
...
Modern turbofan engines generate continuous streams of sensor data throughout their operational life, yet deciding exactly when to perform maintenance remains a difficult problem. Scheduling maintenance too early wastes useful engine life and incurs unnecessary cost, while acting too late risks catastrophic in-flight failure. The challenge is compounded by the reality that sensors degrade over time, injecting noise into the very data that maintenance decisions depend on. Most existing predictive maintenance systems treat their own predictions as ground truth, leaving them vulnerable to silent failures when sensor quality deteriorates.

This project addressed that gap by developing a three-layer hybrid artificial intelligence framework that combines deep learning, reinforcement learning, and deterministic safety logic for turbofan engine maintenance scheduling. Layer~1 employed a deep ensemble of five Long Short-Term Memory networks, each trained on a different bootstrap sample of the NASA C-MAPSS dataset (FD001 and FD002, 360 engines combined). The ensemble predicted Remaining Useful Life while simultaneously quantifying prediction uncertainty through model disagreement. Layer~2 used a Deep Q-Network agent that observed the ensemble mean prediction, instantaneous and rolling uncertainty, and a sensor trend signal. Crucially, the agent was trained with noise augmentation, where seventy per cent of training episodes injected Gaussian sensor noise at varying intensities, teaching the agent that high ensemble disagreement signals unreliable predictions. Layer~3 implemented a two-tier safety supervisor that overrode the agent when the ensemble confidently predicted imminent failure, providing a hard safety guarantee.

The framework was evaluated through a rigorous ablation study comprising three experiments across eight noise levels with 500 episodes per condition. When the safety supervisor was removed, the uncertainty-aware agent retained positive reward at heavy noise while a blind control agent, identical but denied the uncertainty signal, collapsed to negative returns. Under the full three-layer system, the safety supervisor eliminated all catastrophic failures for both agents, but the uncertainty-aware agent achieved a seventy per cent autonomous optimal-timing rate compared to forty-three per cent for the blind agent, demonstrating genuine decision-making superiority rather than reliance on the safety net. A financial analysis projected that deploying the uncertainty-aware strategy across a fifty-engine fleet would yield substantial annual cost savings over the blind approach, driven primarily by fewer wasteful early repairs and the complete elimination of unplanned failure events.

This work demonstrates that making a reinforcement learning agent explicitly aware of its own prediction uncertainty produces more robust and more autonomous maintenance decisions under realistic sensor degradation.
...
Every commercial flight depends on the reliable operation of its engines. A single unplanned engine failure can ground an aircraft for weeks, cost hundreds of thousands of pounds in emergency repairs and passenger compensation, and in the most severe cases endanger lives. Airlines and maintenance organisations have therefore invested heavily in systems that predict when an engine will fail, a discipline known as Predictive Maintenance (PdM), so that repairs can be scheduled at the optimal moment: late enough to extract maximum useful life from each component, yet early enough to prevent catastrophic breakdown.
...
This project tackled that problem directly. Rather than building yet another RUL predictor, it developed a complete decision-making framework that knows when it does not know. The system comprised three cooperating layers: a deep learning ensemble that quantified its own prediction uncertainty, a reinforcement learning agent that learned to interpret that uncertainty signal when choosing whether to wait or perform maintenance, and a deterministic safety supervisor that provided a final hard guarantee against catastrophic failure. The result was a framework that performed comparably to conventional systems under ideal conditions, but degraded far more gracefully when sensor noise was present, precisely the scenario that matters most in real-world deployment.
...
This project aimed to develop and evaluate a three-layer hybrid artificial intelligence framework that integrates uncertainty-aware deep learning, reinforcement learning, and rule-based safety logic for optimal turbofan engine maintenance scheduling under sensor degradation.
...
\begin{enumerate}
  ...
  \item \textbf{Safety supervisor (Layer~3).} Implement a deterministic two-tier safety override that forces maintenance when the ensemble confidently predicts imminent failure, providing a hard safety guarantee against catastrophic engine loss.
...
\addlinespace
Safety as an afterthought & Safety mechanisms are rarely integrated into the RL evaluation; most work assumes the agent will learn safe behaviour on its own. & A two-tier deterministic safety supervisor provides a hard override guarantee, evaluated as a formal ablation layer. \\
\addlinespace
...
The prediction layer comprised five independently trained LSTM networks. Each network consisted of a two-layer LSTM with a hidden dimension of 100 and a dropout rate of 0.2, followed by a single linear output layer that produced a scalar RUL prediction from the last hidden state. The input to each network was a sliding window of 30 consecutive cycles, with each cycle represented by 24 features (21 sensor measurements and 3 operational settings). The RUL target was normalised to the range [0,~1] by dividing by the cap of 125 cycles.

\vspace{0.3cm}

Diversity among ensemble members was achieved through bootstrap sampling: each model was trained on a random 80\% subset of the available engine trajectories, drawn with replacement. This follows the recommendation of \citet{fort2019deep}, who showed that training data diversity, combined with random weight initialisation, is sufficient to produce meaningful disagreement among ensemble members. At inference time, the ensemble produced two signals for each cycle: the mean of the five individual predictions (the point estimate) and the standard deviation across the five predictions (the uncertainty signal). The standard deviation was scaled by a factor of 15 (determined empirically during development) to map the raw range of approximately 0.01--0.10 into a range that was informative for the downstream DQN agent.
...
The two-tier design encoded an important asymmetry. At moderately low RUL values, the supervisor deferred to the DQN agent when uncertainty was high, trusting that the agent's uncertainty-aware training equipped it to handle ambiguous situations. At extremely low RUL values, no amount of uncertainty justified further waiting, and the hard override provided an absolute guarantee against catastrophic failure. This design was directly informed by the shielding literature reviewed in Section~2.4 and the defence-in-depth principle mandated by aerospace safety standards \citep{do178c}.

\vspace{0.3cm}

To illustrate how these three layers operate in concert during a single engine lifecycle, Figure~\ref{fig:engine_timeline} presents a representative timeline showing the ensemble prediction, the uncertainty signal, and the agent's decision pressure evolving cycle by cycle as an engine approaches end of life.
...
\midrule
\texttt{src/config.py} & Centralised configuration for all hyperparameters (FR1--FR7, NF3). \\
\texttt{src/preprocessing.py} & Data loading, RUL calculation, normalisation, and sliding-window generation (FR1). \\
\texttt{src/lstm\_model.py} & LSTM network architecture definition (FR2). \\
\texttt{src/gym\_env.py} & Gymnasium environment, blind ablation variant, and safety supervisor (FR3, FR5, FR6). \\
\texttt{main\_train\_ensemble.py} & LSTM ensemble training with bootstrap sampling (FR2). \\
\texttt{main\_train\_rl.py} & DQN agent training with noise augmentation (FR4). \\
\texttt{main\_experiment\_final.py} & Three-experiment ablation study with statistical analysis (FR7). \\
\texttt{main\_experiment\_threshold.py} & Threshold baseline comparison (FR9). \\
\texttt{main\_cost\_analysis.py} & Fleet-level financial projection (FR8). \\
\texttt{main\_evaluate\_ensemble.py} & LSTM ensemble evaluation and diagnostic plots (FR10). \\
\texttt{main\_visualize.py} & Engine lifecycle timeline visualisation (FR10). \\
\texttt{dashboard.py} & Streamlit interactive demonstration (FR11, FR12). \\
\texttt{data/} & NASA C-MAPSS dataset files (FD001, FD002). \\
\texttt{models/} & Trained model weights and scaler artefacts. \\
\bottomrule
\end{tabular}
\end{table}
...
The safety supervisor implemented FR6 as a pure Python function that accepted the DQN agent's proposed action and the current observation vector and returned a possibly modified action. The implementation was deliberately kept outside any learned component to provide a deterministic guarantee, following the shielding principle formalised by \citet{alshiekh2018safe} and the defence-in-depth requirement of aerospace safety standards \citep{do178c}.

\vspace{0.3cm}

\textbf{Two-tier logic.} As specified in Section~3.5.3, the function implemented two override conditions evaluated in priority order. Tier~1 (hard critical) fired unconditionally when the ensemble mean prediction fell below 0.04 (approximately 5 cycles remaining), providing an absolute last-resort guarantee. Tier~2 (uncertainty-aware) fired when the prediction fell below 0.12 (approximately 15 cycles) \emph{and} the rolling uncertainty was below 0.55, ensuring that the override only engaged when the ensemble was confident in its low-RUL assessment. If neither condition was met, the agent's original action passed through unmodified.

\vspace{0.3cm}

\textbf{Interaction with the two agent types.} The design created an important asymmetry between the uncertainty-aware (UA) and blind agents that was central to the experimental evaluation. The blind agent reported rolling uncertainty of zero at every step, which always satisfied the confidence condition ($0 < 0.55$), causing Tier~1 to fire on every low-RUL prediction, including noise-induced false alarms. The UA agent reported genuine ensemble disagreement, meaning Tier~1 only fired when the ensemble members agreed on a low prediction, effectively filtering out transient noise-driven dips. This difference meant the blind agent was caught by the safety net more frequently but also experienced more unnecessary overrides, while the UA agent operated more autonomously under the safety supervisor's protection.
...
To illustrate the system operating across different engine lifecycles, Figures~\ref{fig:engine_134} and~\ref{fig:engine_200} present the three-panel decision timelines for Engine~134 and Engine~200. These engines were selected because they represent contrasting scenarios: Engine~134 exhibited a relatively smooth degradation profile, while Engine~200 presented a noisier trajectory that tested the agent's uncertainty handling.
...
\midrule
0.000 & 406.4 & 0.0 & 73.4 & 366.4 & 5.0 & 70.6 & * & 0.16 \\
0.025 & 289.8 & 0.0 & 49.8 & 247.5 & 4.4 & 48.0 & * & 0.16 \\
0.050 & 293.8 & 0.0 & 45.4 & 206.6 & 8.2 & 45.0 & *** & 0.31 \\
0.075 & 283.5 & 1.0 & 40.6 & 162.8 & 12.4 & 40.6 & *** & 0.41 \\
0.100 & 233.5 & 3.8 & 32.4 & 45.0 & 26.4 & 31.6 & *** & 0.56 \\
0.125 & 211.6 & 4.6 & 25.4 & 6.9 & 33.4 & 31.4 & *** & 0.59 \\
0.150 & 171.1 & 11.6 & 23.8 & $-58.3$ & 40.8 & 25.4 & *** & 0.61 \\
0.175 & 135.6 & 13.0 & 16.6 & $-112.8$ & 50.4 & 23.8 & *** & 0.65 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
0.000 & 396.6 & 71.4 & 13 & 96.6 & 432.1 & 78.6 & 201 & 59.8 \\
0.025 & 264.0 & 44.0 & 167 & 63.8 & 192.2 & 32.0 & 246 & 46.2 \\
0.050 & 224.0 & 29.6 & 215 & 46.0 & 155.4 & 28.0 & 292 & 33.4 \\
0.075 & 195.9 & 18.2 & 281 & 30.6 & 138.8 & 25.4 & 336 & 25.8 \\
0.100 & 176.4 & 11.4 & 330 & 20.0 & 131.9 & 25.4 & 401 & 12.6 \\
0.125 & 156.5 & 8.2 & 364 & 16.2 & 135.5 & 26.4 & 431 & 10.4 \\
0.150 & 143.7 & 6.2 & 401 & 10.6 & 135.0 & 26.6 & 455 & 6.4 \\
0.175 & 160.2 & 5.0 & 408 & 9.0 & 118.4 & 23.2 & 461 & 6.0 \\
\bottomrule
\end{tabular}
\end{table}
...
The autonomy analysis revealed the most important qualitative finding of the entire evaluation. Under the full system, the UA agent achieved a 70\% autonomous optimal-timing rate on clean data (jackpots triggered by the agent's own decision rather than by the supervisor), compared to 43\% for the blind agent. The autonomous jackpot counts from Table~\ref{tab:exp2_results} show that on clean data, the UA agent produced 350 autonomous jackpots out of 500 episodes, while the blind agent produced only 214. This gap persisted across noise levels: at $\sigma = 0.050$, the UA agent scored 111 autonomous jackpots versus 55 for the blind agent; at $\sigma = 0.100$, 43 versus 20. These figures demonstrate that the UA agent was not merely performing well because the safety supervisor rescued it, but rather because it had genuinely learned to make better timing decisions by conditioning on the uncertainty signal.
...
Under the full three-layer system, the UA strategy produced a lower average cost per maintenance decision than the blind strategy at six out of eight noise levels. The advantage was most pronounced in the moderate noise range ($\sigma = 0.025$ to $\sigma = 0.075$), where the UA agent's ability to distinguish between reliable and unreliable predictions allowed it to time maintenance more accurately, avoiding both the wasteful early interventions and the supervisor-triggered safe-but-early events that inflated the blind agent's costs. At the heaviest noise levels ($\sigma = 0.150$ and above), the safety supervisor dominated both agents' behaviour, narrowing the cost gap as both agents increasingly relied on the override mechanism.

\vspace{0.3cm}

Figure~\ref{fig:final_result} provides a summary visualisation of the key evaluation metrics across all experimental conditions.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{final_result_chart.png}
\caption{Summary of key evaluation metrics across all experimental conditions, showing the systematic advantage of the uncertainty-aware agent in mean reward, failure avoidance, and optimal timing rate compared to the blind baseline.}
...
\begin{enumerate}
  ...
  \item \textbf{The safety supervisor eliminated catastrophic failures.} Under the full system (Experiment~2), both agents achieved near-zero failure rates across all noise levels, validating the two-tier deterministic override design.

  \item \textbf{The UA agent was substantially more autonomous.} The UA agent operated without supervisor intervention in 96.6\% of clean episodes and produced 70\% autonomous jackpots, compared to 59.8\% autonomy and 43\% autonomous jackpots for the blind agent. This gap persisted across all noise levels.
...
\textbf{Objective 5: Safety supervisor.} Fully achieved. The two-tier deterministic override eliminated catastrophic failures in Experiment~2 across all noise levels for the UA agent. The uncertainty-aware design created the intended asymmetry: the blind agent triggered the supervisor far more frequently because its zeroed uncertainty always satisfied the confidence condition, while the UA agent's genuine uncertainty signal allowed more selective intervention \citep{alshiekh2018safe}.
...
\textbf{On clean data, the blind agent outperformed.} The blind agent achieved a higher mean reward (432.1 vs 396.6) on clean data because it operated with high confidence at all times, and under clean conditions its predictions were reliable. The UA agent's conservative bias, trained to respond to mild uncertainty always present even in clean data, reduced its reward under ideal conditions but proved advantageous under noise. This robustness-optimality trade-off is analogous to the bias-variance trade-off \citep{der2009aleatory} and consistent with domain randomisation findings \citep{tobin2017domain}.
...
The experimental evaluation provided strong evidence for the central hypothesis of this project: that making a reinforcement learning agent explicitly aware of its own prediction uncertainty produces more robust and more autonomous maintenance decisions under sensor degradation. The UA agent outperformed the blind agent at six out of eight noise levels in Experiment~1, with the advantage growing systematically from negligible to medium effect sizes as noise increased. Under the full three-layer system, the safety supervisor eliminated catastrophic failures for both agents, but the UA agent achieved substantially higher autonomous decision-making rates, demonstrating genuine policy superiority rather than reliance on the safety net. The threshold baseline comparison confirmed that the learned policy captured non-trivial timing behaviour, and the financial analysis translated these technical advantages into projected fleet-level cost savings.
...
The C-MAPSS dataset used in this project is a publicly available NASA benchmark and contains no personally identifiable information \citep{saxena2008damage}. However, a production deployment would ingest live sensor telemetry from operational engines, fleet identifiers, and maintenance logs. Under the UK Data Protection Act 2018 and the retained EU General Data Protection Regulation (GDPR), such data could constitute personal data if it can be linked to individual crews, routes, or operators. A Data Protection Impact Assessment would therefore be required before deployment, and the system would need to comply with the principles of data minimisation, purpose limitation, and storage limitation. Sensor readings should be anonymised or pseudonymised wherever possible, and role-based access controls must restrict who can view fleet-level patterns that might reveal commercially sensitive operational behaviour.

\section{Aerospace Safety Regulation and Certification}

Aviation software that influences maintenance decisions falls within the scope of RTCA DO-178C, the internationally recognised standard for airborne software assurance \citep{do178c}. DO-178C defines five Design Assurance Levels (DAL A through E); a system that can trigger or defer maintenance actions on flight-critical components would likely be classified at DAL B or above, requiring modified condition/decision coverage testing and formal traceability from requirements to code. The current prototype satisfies none of these certification requirements: it was developed as a research proof of concept, not as certifiable software. Before any real-world deployment, the entire codebase, training pipeline, and safety supervisor logic would need to be re-engineered under a DO-178C-compliant development process, including independent verification and configuration management.

The European Union Aviation Safety Agency (EASA) has also published a roadmap for AI in aviation that mandates explainability and human oversight for any AI system used in safety-critical decisions. The safety supervisor (Layer~3) of this project partially addresses the oversight requirement by providing a deterministic override that can be audited, but the DQN policy itself remains a black-box neural network. Integrating post-hoc explainability methods such as SHAP values or attention-based attribution would be necessary to satisfy regulatory expectations.
...
If the system were to be deployed commercially, several additional factors would need to be addressed. The computational cost of running a five-model LSTM ensemble in real time is modest on modern hardware, but the system would need to be integrated with an airline's existing fleet management software, maintenance scheduling databases, and spare-parts inventory systems. Data ownership would need to be clearly defined: sensor telemetry is typically owned by the engine manufacturer (under power-by-the-hour contracts), while maintenance records may be owned by the operator. The AI system would need contractual access to both data streams, subject to appropriate data-sharing agreements.

Scalability is another concern. The current system was trained and evaluated on the C-MAPSS dataset, which models a single engine type under a limited set of operating conditions. Extending coverage to multiple engine families (e.g., CFM56, LEAP, Trent XWB) would require separate ensemble training for each type, and the RL agent would need to generalise across different degradation profiles. Finally, ongoing model maintenance, including retraining as sensor drift occurs and validating against evolving certification standards, represents a continuous operational commitment that must be factored into any commercial business case.
...
\begin{enumerate}
    ...
    \item \textbf{Safety supervisor evaluation.} Achieved. Zero crash rates under full system; UA agent achieved 70\% autonomous jackpots versus 43\% for blind.
    \item \textbf{Threshold baseline comparison.} Achieved. DQN outperformed all eight baselines across six noise levels.
    \item \textbf{Financial cost analysis.} Achieved. Projected 18.2\% annual cost savings versus threshold-based maintenance.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Two-tier safety supervisor.} The combination of an uncertainty-aware override (fires when predictions are confident and low) with a hard-critical fallback (fires unconditionally below 5 cycles) provided defence-in-depth safety that eliminated catastrophic failures across all experimental conditions.
...
\begin{itemize}
    \item The C-MAPSS dataset, while widely used, is a simulation. Real engine sensors exhibit non-Gaussian noise, intermittent faults, and telemetry dropouts that the Gaussian noise injection model does not capture.
    \item The binary action space (wait or maintain) is a simplification. Real maintenance scheduling involves partial inspections, deferred actions, and fleet-level coordination that a richer action space would better represent.
    \item The UA agent sacrificed approximately 10 reward points under clean conditions (Table~\ref{tab:exp2_results}), indicating a robustness-optimality trade-off that may not be acceptable in all operational contexts.
    ...
    \item All financial projections were based on simulated outcomes and industry-representative cost figures rather than actual airline maintenance records.
\end{itemize}
...
\midrule
0.000 & 73.4 & 70.6 & 500.0 & 501.4 & 2.545 & * \\
0.025 & 49.8 & 48.0 & 487.0 & 146.6 & 2.553 & * \\
0.050 & 45.4 & 45.0 & 468.6 & 93.7 & 4.934 & *** \\
0.075 & 40.6 & 40.6 & 452.2 & 45.0 & 6.438 & *** \\
0.100 & 32.4 & 31.6 & 133.2 & 16.0 & 8.897 & *** \\
0.125 & 25.4 & 31.4 & 88.0 & 14.0 & 9.275 & *** \\
0.150 & 23.8 & 25.4 & 77.5 & 10.0 & 9.642 & *** \\
0.175 & 16.6 & 23.8 & 52.0 & $-$314.5 & 10.255 & *** \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
0.000 & 13 & 201 & 96.6 & 59.8 & 350 & 214 \\
0.025 & 167 & 246 & 63.8 & 46.2 & 173 & 69 \\
0.050 & 215 & 292 & 46.0 & 33.4 & 111 & 55 \\
0.075 & 281 & 336 & 30.6 & 25.8 & 70 & 41 \\
0.100 & 330 & 401 & 20.0 & 12.6 & 43 & 20 \\
0.125 & 364 & 431 & 16.2 & 10.4 & 34 & 18 \\
0.150 & 401 & 455 & 10.6 & 6.4 & 17 & 12 \\
0.175 & 408 & 461 & 9.0 & 6.0 & 20 & 11 \\
\bottomrule
\end{tabular}
\end{table}
...
Appendix~\ref{app:visualisations} presents supplementary figures that provide additional perspectives on the experimental results. Figure~\ref{fig:app_exp1_alt} shows the Layer~2 ablation from an alternative angle, while Figure~\ref{fig:app_exp2_alt} provides an alternative view of the full system performance.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.85\textwidth]{experiment_1_layer2_ablation.png}
    \caption{Experiment 1 Layer 2 ablation: alternative visualisation of UA vs Blind reward distributions across all eight noise levels without the safety supervisor.}
...
\begin{figure}[h]
    ...
    \caption{Experiment 2 full system: alternative visualisation of UA vs Blind performance with the three-layer safety supervisor active across all noise levels.}
