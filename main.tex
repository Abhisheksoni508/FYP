...
\textbf{Single primary RL algorithm.} The main experiments used DQN exclusively. A supplementary PPO comparison (Appendix~B.4) was conducted to verify algorithm choice; however, a full multi-algorithm study with equivalent training budgets would strengthen the generality of the conclusions.
...
\begin{itemize}
    ...
    \item \textbf{Extended RL algorithm comparison.} The supplementary PPO comparison (Appendix~B.4) confirmed DQN's suitability for the binary action space. A more comprehensive study with equivalent training budgets across PPO \citep{schulman2017proximal}, SAC \citep{haarnoja2018soft}, and Double DQN could reveal whether policy-gradient methods handle the uncertainty dimension more effectively under richer action spaces.
...
The results revealed a nuanced picture. PPO achieved a higher average jackpot rate across conditions (62.4\% vs 50.5\%), but this came at the cost of substantially higher failure rates, particularly on clean data (18.3\% vs 0.0\%). PPO learned a more aggressive policy: when it timed correctly it scored more jackpots, but when it misjudged it crashed more often. DQN's off-policy replay mechanism produced a more conservative and reliable policy, which aligned better with the project's safety-oriented objectives.

Critically, both algorithms retained positive mean reward at all noise levels, confirming that the uncertainty-awareness mechanism was not an artefact of the DQN algorithm. The PPO agent, given access to the same uncertainty signal and trained with the same noise augmentation, also learned to condition its behaviour on $\sigma$, supporting the generality of the core contribution. DQN remained the better choice for this specific setting because the binary action space and expensive ensemble-inference environment favoured off-policy sample efficiency over PPO's on-policy updates. Figure~\ref{fig:ppo_comparison} visualises the comparison.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{report_ppo_vs_dqn.png}
\caption{Algorithm comparison: DQN vs PPO, both uncertainty-aware, evaluated without the safety supervisor across four noise levels. PPO achieved higher jackpot rates but also higher failure rates, reflecting a more aggressive learned policy. Both algorithms retained positive mean reward under noise, confirming the generality of the uncertainty-awareness mechanism.}
\label{fig:ppo_comparison}
\end{figure}

Figure~\ref{fig:ppo_statistical} provides a deeper statistical analysis. The per-noise reward delta shows that DQN's advantage was significant only on clean data ($p = 0.019$), while PPO held a significant advantage at $\sigma = 0.05$ ($p < 0.001$). Effect sizes were small to negligible across all conditions ($|d| < 0.36$), indicating that neither algorithm was decisively superior. The jackpot-versus-failure breakdown confirms the core trade-off: PPO's aggressive timing policy produced more optimal-timing successes but also more catastrophic failures.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{report_ppo_statistical.png}
\caption{Statistical analysis of DQN vs PPO comparison. Left: per-noise reward delta with significance markers (* $p < 0.05$, *** $p < 0.001$, n.s.\ not significant). Centre: Cohen's $d$ effect sizes with conventional thresholds. Right: jackpot rate (bars) and failure rate ($\times$ markers) by algorithm and noise level.}
