...
This project aimed to develop and evaluate a three-layer hybrid artificial intelligence framework that integrates uncertainty-aware deep learning, reinforcement learning, and rule-based safety logic for robust maintenance scheduling of \emph{any IoT-monitored asset} under sensor degradation. The framework is deliberately domain-agnostic: the observation space (normalised RUL, instantaneous uncertainty, rolling uncertainty, and sensor trend) and the safety supervisor's threshold logic contain no asset-specific assumptions. Layer~1 (the prediction model) is the only component that needs retraining for a new asset class; Layers~2 and~3 transfer unchanged because they operate on abstract health and uncertainty signals.

\paragraph{Why C-MAPSS?} A general framework still needs a specific validation case, and C-MAPSS was chosen for four reasons. (1)~\emph{Benchmark status.} C-MAPSS is the most widely used run-to-failure dataset in the prognostics literature \citep{saxena2008damage}, used in hundreds of published studies, which makes prediction quality directly comparable to prior work. (2)~\emph{Difficulty.} Turbofan engines are a demanding, safety-critical IoT case with high-dimensional, noisy multivariate sensor streams, the same data characteristics found in wind turbines, CNC spindles, HVAC compressors, and pump fleets. An approach that works under C-MAPSS noise has earned a defensible claim that it will work under those analogous conditions. (3)~\emph{Ground truth available.} Real industrial PdM data rarely contains true RUL labels because operators perform preventive maintenance long before failure. C-MAPSS is a run-to-failure simulation, so every cycle has a known true RUL, which is essential for training and for honest evaluation. (4)~\emph{Reproducibility.} It is public, which lets the experiments be replicated by anyone. Real proprietary IoT data would have made the project unverifiable. The framework's transfer to other IoT domains is recorded as a top-line item in Future Work (Section~7.4); the architectural reasoning behind that transferability is given in Section~3.5.
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{LSTM ensemble (Layer~1).} Train five bootstrap-sampled LSTMs, validate against published baselines, and check that ensemble disagreement tracks prediction error.
  \item \textbf{Gymnasium environment.} Formulate maintenance as an MDP and implement a custom environment exposing prediction, uncertainty, and trend to the agent.
...
\textbf{Development methodology.} Iterative bottom-up prototyping. Layer~1 trained and validated first; Layer~2 built on confirmed predictions and tuned iteratively; Layer~3 integrated last as a deterministic module. Each layer depended on the correctness of the one below it.
...
The Commercial Modular Aero-Propulsion System Simulation (C-MAPSS) dataset, released by NASA's Prognostics Center of Excellence, has become the dominant benchmark in the RUL prediction community \citep{saxena2008damage}. The dataset simulates run-to-failure degradation trajectories of turbofan engines, with each engine recording 21 sensor measurements and 3 operational settings at every cycle. Four subsets (FD001 through FD004) provide progressively more challenging conditions: FD001 features a single operating condition and a single fault mode, while FD004 combines six operating conditions with two fault modes. The widespread adoption of C-MAPSS enables meaningful cross-study comparison, though it remains a simulation and therefore cannot fully capture the noise characteristics and failure modes of real-world engines \citep{ramasso2014review}. Recent surveys have emphasised that bridging the gap between simulated benchmarks and real operational data remains one of the central challenges in prognostics and health management \citep{li2023prognostics, arias2024survey}.
...
Uncertainty quantification has seen limited application in predictive maintenance. \citet{kraus2019deep} applied MC Dropout to LSTM-based RUL prediction on C-MAPSS and showed uncertainty correlated with error but did not feed it into a decision layer. \citet{peng2020bayesian} trained a Bayesian LSTM for turbine blade RUL and observed rising epistemic uncertainty away from the training distribution. In both cases uncertainty was used as a diagnostic for human interpretation rather than as input to an automated downstream system. This project closes that loop by feeding ensemble uncertainty directly into the RL observation space.


Figure~\ref{fig:ensemble_pred} shows predicted versus true RUL on the held-out test set, and Figure~\ref{fig:ensemble_uncertainty} shows ensemble disagreement correlating with prediction error, confirming the uncertainty signal is well-calibrated.
...
A growing body of work uses prediction uncertainty to trigger cautious behaviour. \citet{michelmore2020uncertainty} used epistemic uncertainty for human handover in autonomous driving. \citet{lutjens2019safe} constrained agent actions when a Bayesian network reported high uncertainty during navigation. \citet{liu2023safe} extended uncertainty-aware exploration to systems with formal safety constraints. \citet{chen2024uncertainty} applied deep ensemble uncertainty to industrial predictive maintenance, providing independent validation that ensemble disagreement is an effective timing signal. This project is best understood as a simplified integrated implementation of these ideas: ensemble uncertainty as an RL observation, noise-augmented training, and a deterministic safety supervisor assembled into one pipeline on a standard benchmark, rather than the invention of a new safety mechanism.
...
This project addresses all five gaps together: a deep LSTM ensemble for RUL and uncertainty (Section~2.2), a DQN that observes both (Section~2.3), noise-augmented training so the agent learns what uncertainty means, a two-tier safety supervisor as a deterministic backstop (Section~2.4), and an ablation study across eight noise levels. The resulting design principles (LSTM backbone, deep ensembles, DQN, noise augmentation, deterministic shielding) are formalised into requirements in Chapter~3.
...
Requirements were drawn from the eight project objectives (Section~1.2), state-of-the-art patterns in the literature review, C-MAPSS dataset constraints, and practical issues discovered during iterative prototyping. No user studies were conducted: this is a research prototype evaluated by simulation.
...
\begin{enumerate}[label=\textbf{A\arabic*.}, leftmargin=2cm]
  ...
  \item \textbf{Gaussian noise model.} Zero-mean additive Gaussian, the standard assumption \citep{wang2020remaining}. Real sensor failures (bias, drift, dropout) would need a richer model.
...
Three architectures were compared before the LSTM was adopted, on three axes. (a) Input fit: with a 30-cycle window of 24 features, all three are viable, but LSTMs remain a default for short multivariate sensor sequences \citep{hochreiter1997long, zheng2017long}; Transformers \citep{vaswani2017attention, zhang2022transformer} pay off mainly on longer sequences; 1D-CNNs \citep{li2018remaining} are efficient but weaker on asymmetric degradation curves. (b) Ensemble cost on commodity hardware: the LSTM ensemble trained in roughly 25 minutes on a single consumer GPU, whereas a comparable Transformer ensemble was estimated at two to three hours from pilot runs, which would have squeezed the RL experiment budget. (c) UQ maturity: deep ensembles are well characterised for LSTMs \citep{lakshminarayanan2017simple, fort2019deep}, whereas Transformer-ensemble calibration on small datasets is less well understood. LSTM was therefore the pragmatic choice, not an endorsement of intrinsic superiority. A Transformer backbone is listed as follow-up work in Chapter~7.

Each of the five networks is a two-layer LSTM (hidden dimension 100, dropout 0.2) with a linear output head that maps the last hidden state to a scalar RUL. Inputs are 30-cycle windows of 24 features; targets are normalised by the 125-cycle cap. Diversity comes from bootstrap sampling: each model trains on a random 80\% subset \citep{fort2019deep}. Deep ensembles were chosen over MC Dropout because they are better calibrated under shift \citep{lakshminarayanan2017simple, ovadia2019can}. At inference, each cycle yields a mean and standard deviation:
\begin{equation}
...
The binary action space is a conscious simplification, not an oversight. It was chosen because (a) it keeps the reward signal interpretable, with each episode ending in a clearly labelled outcome (jackpot, safe, wasteful, or crash), (b) it enables clean ablation, since any gap between UA and blind agents can be attributed to the uncertainty signal rather than to action-space handling, and (c) it isolates the uncertainty-awareness contribution, which is the central hypothesis. The simplification excludes graduated responses (inspection, partial repair, deferred maintenance) and fleet-level coordination, so the findings generalise to binary act-or-wait decisions with asymmetric cost but not to full maintenance planning. This trade-off is revisited in the Chapter~5 limitations and Chapter~7 future work.
...
The symmetric $\pm 500$ jackpot/crash pair dominates the discounted WAIT stream, forcing the agent to chase optimal timing rather than play safe with early maintenance. Time-pressure shaping (linear decay below RUL 25) blocks a ``wait forever'' policy. Two uncertainty terms close the loop: an urgency penalty when the agent waits in the buffer zone with high $\sigma$, and a $+30$ proactive bonus when it maintains in the same regime. Together they encode the rule that uncertain end-of-life states warrant action, not inaction.
...
\textbf{Algorithm choice: DQN vs PPO, SAC, and advanced DQN variants.} The RL algorithm was selected after an explicit comparison on four criteria. (a) Action-space fit: the binary discrete action (WAIT/MAINTAIN) is the natural home of value-based methods \citep{mnih2015human}; PPO \citep{schulman2017proximal} and SAC \citep{haarnoja2018soft} target continuous or high-dimensional actions and offer no structural advantage here. (b) Sample efficiency: experience replay lets DQN reuse the expensive ensemble-inference transitions many times, which matters because every step runs five LSTM forward passes; on-policy PPO would discard data after each update and slow training under the same hardware budget. (c) Stability: Double DQN \citep{vanhasselt2016deep}, Duelling DQN \citep{wang2016dueling}, and distributional variants such as C51 \citep{bellemare2017distributional} are known to improve stability in other domains. Vanilla DQN was used as the baseline because the project needed a clean, well-understood reference against which the uncertainty-awareness contribution could be isolated; swapping in a stronger variant and the new observation together would have muddied the ablation. A fuller comparison is listed as future work. (d) Implementation maturity: Stable-Baselines3 \citep{raffin2021stable} provides a well-tested DQN implementation, reducing bug risk in a loop that already sits on top of a custom environment and ensemble. The supplementary PPO comparison in Appendix~B.4 partially addresses the broader algorithm concern by showing DQN produced a safer risk-adjusted policy than PPO under identical conditions. The core DQN target and loss are:
\begin{equation}
...
To make Figure~\ref{fig:component_diagram} concrete, this section walks through what happens to a single engine cycle as it travels from raw sensor reading to a final maintenance decision. Every numbered step corresponds to one block in the diagram.

\begin{enumerate}
    \item \textbf{A new cycle's sensor reading arrives.} The engine reports 24 numbers: 21 physical sensor measurements (temperatures, pressures, fan speeds) and 3 operational settings (altitude, throttle, Mach). At this point the values are in their original physical units and on wildly different scales, so they cannot yet be fed into a neural network.

    \item \textbf{Normalisation.} The reading is passed through the saved \texttt{MinMaxScaler} (fitted once during training) which squashes every feature into the $[0, 1]$ range. The same scaler is always reused so that what the model saw during training matches what it sees in deployment.

    \item \textbf{Sliding window assembly.} The system stitches the new normalised reading onto the previous 29 cycles to form a $30 \times 24$ matrix. This gives the LSTMs a short rolling history of the engine's recent behaviour rather than just one isolated snapshot, which is essential for spotting degradation trends.

    \item \textbf{(Training only) Noise injection.} During training (and only during training) 70\% of episodes have a small Gaussian noise added on top of the window. This is the trick that teaches the system what unreliable data looks like. At deployment time this block is switched off entirely.

    \item \textbf{Five LSTMs predict the RUL in parallel.} The window is fed simultaneously into all five ensemble members. Each LSTM was trained on a slightly different bootstrap sample of the data, so they have learned slightly different ``opinions'' of what healthy and degraded engines look like. Each one outputs its own estimate of how many cycles the engine has left.

    \item \textbf{Aggregate the five opinions.} The five predictions are reduced into two numbers: their average ($\bar{r}_t$, the system's best guess of the remaining life) and their standard deviation ($\sigma_t$, how much they disagree). The disagreement is the uncertainty signal: when the LSTMs disagree, the system knows its prediction is shaky.

    \item \textbf{Build the observation vector.} The DQN agent doesn't just see the raw RUL guess. It sees four signals packed into one observation: the mean RUL, the current cycle's $\sigma$, a 3-cycle rolling average of $\sigma$ (to smooth out one-off spikes), and a sensor trend term that summarises whether overall sensor activity is rising or falling. These four numbers together tell the agent both what it predicts and how confident it should be.

    \item \textbf{The DQN proposes an action.} The observation goes through a 3-layer MLP that outputs two Q-values: one for WAIT and one for MAINTAIN. The agent picks whichever is higher. This is the Layer~2 decision.

    \item \textbf{The safety supervisor gets a veto.} Before the action is executed, it passes through Layer~3. If the DQN said MAINTAIN, the supervisor lets it through unchanged. If it said WAIT, the supervisor checks two safety rules: \emph{Tier~1} forces MAINTAIN if RUL is critically low (under 5 cycles) regardless of confidence, and \emph{Tier~2} forces MAINTAIN if RUL is low (under 15 cycles) \emph{and} the rolling $\sigma$ is below the confidence threshold. This $\sigma$-gate is what makes the supervisor smart: it only intervenes when the prediction is trustworthy.

    \item \textbf{The final action is executed.} Whatever action survives the supervisor (whether the DQN's original choice or a forced override) is the maintenance decision for this cycle. If the action is MAINTAIN the episode ends and the system computes the outcome reward; if it is WAIT the engine advances one cycle and the whole loop starts again from step~1.
\end{enumerate}
...
A safety supervisor can easily mask a weak RL policy, so this question has to be confronted directly. For the UA agent, the supervisor fired on roughly 7 episodes out of 500 on clean data (about 1.4\%), while the agent produced 353 autonomous jackpots before the supervisor had any chance to intervene. The RL policy is doing the work; the supervisor is a backstop. For the blind agent the pattern inverts: the supervisor fired on roughly 215 episodes (about 43\%) with only 214 autonomous jackpots. The blind agent is essentially passive, and most of its apparent safety comes from the shield. Under the UA condition, system success is therefore driven by the RL policy rather than by the safety net.
...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.6cm} >{\raggedright\arraybackslash}p{4.6cm} >{\raggedright\arraybackslash}p{7.8cm}}
\toprule
\textbf{Obj.} & \textbf{Outcome} & \textbf{Evidence} \\
\midrule
1 & Data pipeline operational & FD001+FD002 (360 engines), RUL cap 125, $\sim$46k sequences \citep{heimes2008recurrent}. \\
...
6 & Ablation evaluation & 24,000 episodes, Welch's $t$-test and Cohen's $d$ at every condition; far above the C-MAPSS RL norm \citep{zhang2019deep}. \\
...
CNN-LSTM & \citet{wang2024adaptive} & 12.31 \\
\midrule
...
\textbf{Strengths.} The most compelling finding is qualitative, not quantitative. Under the full system the blind agent achieved a slightly higher clean-data reward (432.1 vs 396.6), but it did so through 201 supervisor rescues against 13 for UA. Stripped of the supervisor (Experiment~1), the blind agent collapsed and the UA agent did not. The UA policy is what produces safety; the blind policy is what \emph{forces the supervisor} to produce safety. For deployment that distinction matters: a system safe through its own intelligence has a wider operating envelope than one safe through external rescue. The same dynamic explains the conservative bias of the UA agent on clean data (a robustness-optimality trade-off analogous to bias-variance \citep{der2009aleatory}, consistent with domain randomisation findings \citep{tobin2017domain}).


\textbf{Convergence at high noise.} Beyond $\sigma \geq 0.125$ the reward gap narrows ($p = 0.131$, $p = 0.528$) because the supervisor increasingly dominates both agents. This is a design feature, not a failure: the supervisor was explicitly intended to take over when conditions exceed the agents' training envelope.
...
Direct head-to-head comparison is limited because few published pipelines combine exactly the same ingredients. \citet{zhang2019deep} used DQN for C-MAPSS maintenance with no uncertainty, no noise evaluation, and fewer than 50 episodes. This project extends their setup with ensemble uncertainty, noise-augmented training, and a deterministic safety supervisor, evaluated with substantially stronger statistical evidence (500 episodes per condition, eight noise levels). The ensemble uncertainty calibration is consistent with \citet{ovadia2019can}, and the two-tier safety supervisor adapts the shielding idea of \citet{alshiekh2018safe} by conditioning the override on ensemble confidence, a variation motivated by the specific failure modes of uncertainty estimates under sensor noise rather than a claim to a fundamentally new safety algorithm.
...
\textbf{Single primary RL algorithm.} A supplementary PPO comparison (Appendix~B.4) shows DQN winning on risk-adjusted reward (PPO reached up to 18.3\% failure rate on clean data vs DQN's 0\%). A broader multi-algorithm sweep with equal training budgets would strengthen generality.
...
\textbf{Ensemble calibration.} Appendix~H.1 shows the ensemble is mildly overconfident (PICP 0.60 at nominal 0.95). The DQN consumes $\sigma$ as a relative signal, so the trained policy is unaffected, but a production deployment should add a recalibration step before exposing $\sigma$ as a probability.
...
These issues are part of the contribution, not a side note. Several shaped the architecture from the start: the transparent supervisor for accountability and certification, the dashboard for human oversight, and noise-augmented training as a deliberate choice favouring resilience over best-case performance. Even so, the gap between this prototype and a deployable system is large: DO-178C certification, liability arrangements, DQN explainability, and integration with real operational data pipelines all require work that sits as much in regulation and ethics as in engineering.
...
The novelty of this project lies in integration and application, not in the invention of new algorithms. Each building block (LSTM prediction, deep ensembles, DQN, shielding) is well established; the contribution is how they were combined, trained, and evaluated as a single safety-critical pipeline.
...
There are several directions a follow-up project could take from here:
...
The biggest thing I took away from this project is that getting the technical pieces working was only half the work, and the easier half. The first DQN I trained looked fine in casual evaluation, but the moment I started running 500-episode experiments under noise it became obvious that "good enough on a few runs" was hiding a policy that crashed engines under conditions the training distribution had not properly covered. After that I stopped trusting any result that came from fewer than a few hundred episodes, and I started checking effect sizes and confidence intervals before believing my own claims.

The three layers were not designed up front in a single sitting. Layer~1 came first because RUL prediction is the obvious starting point. Layer~2 came after I realised that a prediction on its own does not tell you when to act, which is the actual problem. Layer~3 came after I watched the agent occasionally make catastrophic decisions in edge cases, and accepted that I was not going to engineer those out of the policy directly. Each layer existed because the previous one had a failure mode I could not solve in the layer above it, and that is the part of the architecture I am most pleased with.

The unresolved question I am leaving is the gap between a prototype that works in simulation and a system anyone would actually trust on a real engine. The safety supervisor brings the crash rate to zero across thousands of simulated episodes, but a real regulator would want formal verification, not statistical evidence. Closing that gap is what I would want to work on next if I carried this forward.
...
\begin{figure}[htbp]
    ...
    \caption[Side-by-side dashboard comparison: UA vs Blind on Engine 50]{Side-by-side comparison on Engine~50 (FD001, 198 cycles) with sensor noise $\sigma = 0.05$ and the safety supervisor disabled. The Uncertainty-Aware (UA) agent autonomously triggers \textsc{safe} maintenance at cycle~144 (predicted RUL~$=1$, true RUL~$=24$), while the Blind agent panics at cycle~41 with a true RUL of 125 cycles, an extreme \textsc{early} failure. The Layer~2 decision pressure trace (right) shows the UA agent steadily building toward $Q(\text{MAINTAIN}) - Q(\text{WAIT}) > 0$ as the engine degrades.}
...
\begin{figure}[htbp]
    ...
    \caption[Optimal autonomous decision (\textsc{jackpot}) on Engine 24]{Layer~2 (DQN) maintaining within the optimal RUL${<}20$ window with zero supervisor overrides. Engine~24 (FD001, 147 cycles), clean sensor data, UA agent. Maintenance is triggered at cycle~102 with predicted RUL~$=17$ and true RUL~$=15$, a textbook \textsc{jackpot}. The ensemble disagreement trace remains well below the supervisor threshold (0.55) throughout, demonstrating that the agent is making the decision on its own merits rather than under safety-net coercion.}
...
\begin{figure}[htbp]
    ...
    \caption[UA agent under heavy noise on FD002]{UA agent on Engine~134 from FD002 (205 cycles, six operating conditions) with heavy sensor noise $\sigma = 0.10$. Despite ensemble uncertainty saturating near 1.0 throughout the lifecycle (peak $\Sigma = 49.8$), the UA agent still achieves a perfect \textsc{jackpot} at cycle~175 (predicted RUL~$=0$, true RUL~$=0$) without supervisor assistance. This validates the noise-augmentation training strategy: high-$\sigma$ episodes during training taught the agent to commit decisively even when its inputs are unreliable, rather than freezing or panic-maintaining.}
...
This appendix presents three additional empirical analyses produced in response to supervisory feedback. The first examines whether the ensemble's uncertainty estimates are actually reliable (calibration). The second opens up the learned DQN policy to explain \textit{why} the uncertainty-aware (UA) agent behaves differently from the blind agent. The third documents the training dynamics of both layers, including an honest account of the tuning process.

\section*{H.1 Calibration of Ensemble Uncertainty}
\label{sec:app_calibration}

Calibration is a precondition for the entire architecture to be meaningful. If the ensemble's standard deviation $\sigma$ does not track the true error of the mean prediction, then the DQN's observation vector contains a misleading signal, the safety supervisor's $\sigma$-gate becomes brittle, and the $+136$ vs $-113$ gap reported in Experiment~1 could be an artefact rather than a real effect of uncertainty awareness. Checking calibration is therefore not an optional extra, it is a sanity check on the foundation of the system.
...
\caption{Calibration of the five-member LSTM deep ensemble. Left: reliability diagram plotting empirical coverage against nominal coverage (the diagonal is perfect calibration). Right: PICP at four nominal coverage levels ($50\%$, $80\%$, $90\%$, $95\%$). Computed on the held-out C-MAPSS validation split.}
\label{fig:app_calibration}
\end{figure}

Empirical PICP values at the four nominal targets are $0.20$, $0.41$, $0.52$ and $0.60$ respectively. The curve sits consistently below the diagonal across the full coverage range, indicating that the ensemble's predictive intervals are systematically too narrow: it is overconfident. A nominal 95\% interval only captures 60\% of the held-out RUL values. This is a known limitation of small deep ensembles in high-dimensional regression tasks. \citet{lakshminarayanan2017simple} report that deep ensembles are better calibrated than MC-Dropout but still underestimate tail uncertainty without temperature scaling, and \citet{ovadia2019can} observe similar miscalibration under distribution shift.

The honest implication for the rest of the system is that the \textit{absolute} value of $\sigma$ should not be interpreted as a calibrated probability. What the DQN actually consumes, however, is not a calibrated interval but a \textit{relative} signal: the agent learns that high $\sigma$ correlates with unreliable predictions and that low $\sigma$ correlates with reliable ones. The decision boundary analysis in Section~H.2 confirms this is exactly the structure the agent learns. The supervisor's $\sigma$-gate (\texttt{SUPERVISOR\_SIGMA\_THRESHOLD} $= 0.55$) was tuned empirically against this raw $\sigma$ scale, not against a nominal probability, so the underconfidence does not invalidate it. A production deployment would, however, want to apply a recalibration step (isotonic regression or temperature scaling on a held-out split) before exposing $\sigma$ as a probability to downstream components, and this is recorded as a limitation in Chapter~7.

\section*{H.2 Policy Behaviour Under Uncertainty}
\label{sec:app_policy}

Outcome metrics (reward, jackpot rate, crash rate) tell us \textit{that} the UA agent outperforms the blind agent but not \textit{why}. To answer the why, the policy itself needs to be opened up and visualised. The approach taken here is to sweep the two most interpretable dimensions of the DQN observation, the predicted mean RUL $\mu$ and the current ensemble disagreement $\sigma_{\text{now}}$, while holding the other two dimensions ($\sigma_{\text{rolling}}$, sensor trend) fixed at representative values. The resulting grid is fed through the trained Q-network and the selected action (or the Q-value margin) is plotted as a heatmap. This produces a direct picture of the learned decision boundary in the $(\mu, \sigma)$ plane.
...
\textbf{What the figure shows.} The blind agent's boundary is, as expected, essentially a vertical line: its decision depends only on the predicted mean RUL, because $\sigma$ is zeroed out before it reaches the observation. This is the definition of a blind policy made visible. The UA agent's boundary, by contrast, bends. At low $\sigma$ it looks similar to the blind agent, triggering MAINTAIN only when predicted RUL drops into the late-life zone. As $\sigma$ grows, the MAINTAIN region expands leftward: the agent becomes willing to maintain earlier, even when the mean prediction says there is still life left, because the prediction itself is no longer trusted. That bending \textit{is} the $\sigma$-conditional policy. It is the direct empirical answer to the supervisor's question about why the two agents behave differently.
...
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{p{1.2cm} p{2.2cm} p{2.0cm} p{1.8cm} p{1.8cm} p{3.0cm}}
\toprule
\textbf{Ex.} & \textbf{$\mu_{\text{RUL}}$ (norm)} & \textbf{$\sigma$ (scaled)} & \textbf{UA action} & \textbf{Blind action} & \textbf{Interpretation} \\
\midrule
...
\textbf{Connection to outcomes.} Example~2 is precisely the regime that produces the reward gap in Experiment~1 at noise level $\sigma=0.15$: the UA agent books a safe or jackpot maintenance, the blind agent keeps waiting and eventually crashes. Multiplied across 500 episodes, these individual decisions aggregate into the $+136$ vs $-113$ difference in mean episodic reward. The heatmap is not a separate result, it is the mechanism behind the headline numbers.

\section*{H.3 Training Dynamics}
\label{sec:app_training}

Training-time behaviour is often omitted from project write-ups but was specifically requested as evidence of implementation depth. Figure~\ref{fig:app_training} shows the DQN reward curve over the full 1.5M timesteps of training alongside the per-epoch validation loss for each of the five LSTM ensemble members.
...
\caption{Training dynamics of the two learned layers. Left: DQN episodic reward as a function of environment timestep, smoothed with a rolling mean. Right: per-epoch validation loss for each of the five LSTM ensemble members (seeds $42 + i \times 17$).}
\label{fig:app_training}
\end{figure}

\textbf{LSTM training.} All five members follow the same qualitative loss trajectory: rapid initial drop over the first 10--15 epochs, then a long tail of smaller improvements until early stopping fires (patience 15, typically around epoch 50 of the 80-epoch budget). The curves are close to each other but not identical, which is the desired behaviour for an ensemble: the bootstrap-sampled 80\% subsets and the seed offsets produce enough diversity that predictions disagree in difficult regions, while the overall learning dynamic remains stable. If one member had diverged or plateaued much earlier than the others, ensemble disagreement would mostly reflect model pathology rather than genuine epistemic uncertainty. The figure shows that this failure mode did not occur.

\textbf{DQN training.} The reward curve shows the three phases that are expected of a DQN run with a long $\varepsilon$-greedy schedule. Below the 10k-step replay-start threshold the agent is effectively random and reward is noisy around a low baseline. As $\varepsilon$ decays from $1.0$ to $0.03$ over the first 40\% of training, the agent discovers that maintaining near end-of-life yields the $+500$ jackpot and learns to avoid the $-500$ crash penalty, producing the sharp rise in mean reward. The remaining 60\% of training is stabilisation: the curve wobbles inside a tight band as the replay buffer cycles and the target network synchronises every 1000 steps. Final performance sits in the same range reported by the evaluation runs in Chapter~5, which is the sanity check that the training loop and the evaluation loop agree.

\textbf{Honest engineering note.} This project did not use formal hyperparameter sweeps such as Optuna or population-based training. The DQN hyperparameters are Stable-Baselines3 defaults with targeted manual adjustments informed by inspecting reward curves on short pilot runs (for example, increasing the crash penalty from the initial $-100$ to $-500$ after observing that the discounted present value was being outweighed by accumulated WAIT rewards, as documented in Appendix~G.3). The LSTM hyperparameters follow common C-MAPSS conventions. A proper sweep would almost certainly yield small additional gains, and this is listed as a limitation in Chapter~7. The point of showing these curves is not to claim optimality but to demonstrate that the training process was actually observed, reasoned about, and iteratively adjusted rather than treated as a black box.
